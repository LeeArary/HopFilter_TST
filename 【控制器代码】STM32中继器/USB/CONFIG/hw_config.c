...
/*******************************************************************************
* Function Name : Relay_Send.
* Description   : prepares buffer to be sent containing Relay event infos.
* Input         : Keys: keys received from terminal.
* Output        : None.
* Return value  : None.
*******************************************************************************/
void Relay_Send(u8 buf0,u8 buf1,u8 buf2,u8 buf3)
{
...
void USB_Port_Set(u8 enable)
{
	...
	if(enable)_SetCNTR(_GetCNTR()&(~(1<<1)));//退出断电模式：控制寄存器的 PDWN位
	else
	{	  
...
void Get_SerialNum(void)
{
 ...
 //数值转为字符
  if (Device_Serial0 != 0)
  {
    IntToUnicode (Device_Serial0, &Relay_StringSerial[2] , 8);
    IntToUnicode (Device_Serial1, &Relay_StringSerial[18], 4);
  }
}
